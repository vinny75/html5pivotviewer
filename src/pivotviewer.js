...
(function($) {
                    ...
                    if (currentItemFacet.Name == currentFacetCategory.Name) {
                        for (var k = 0; k < currentItemFacet.FacetValues.length; k++) {
                            if (currentFacetCategory.Type == PivotViewer.Models.FacetType.String ||
                                ...
                                if (!found) {
                                    _facetItemTotals.push({
                                        itemId: itemId,
                                        itemValue: currentItemFacet.FacetValues[k].Value,
                                        facet: currentItemFacet.Name,
                                        count: 1
                                    });
                                }
                                ...
                                if (!numFound) {
                                    _facetNumericItemTotals.push({
                                        Facet: currentItemFacet.Name,
                                        Values: [currentItemFacet.FacetValues[k].Value]
                                    });
                                }
                            }
                        }
                ...
                if (!hasValue) {
                    ...
                    if (!found) {
                        _facetItemTotals.push({
                            itemId: itemId,
                            itemValue: "(no info)",
                            facet: currentFacetCategory.Name,
                            count: 1
                        });
                    }
                }
                ...
                if (currentFacetCategory.IsWordWheelVisible) {
                    ...
                    for (var j = 0, _jLen = currentItem.Facets.length; j < _jLen; j++) {
                        ...
                        if (currentItemFacet.Name == currentFacetCategory.Name) {
                            for (var k = 0; k < currentItemFacet.FacetValues.length; k++) {
                                if (currentFacetCategory.Type == PivotViewer.Models.FacetType.String) {
                                    _wordWheelItems.push({
                                        Facet: currentItemFacet.Name,
                                        Value: currentItemFacet.FacetValues[k].Value
                                    });
                                }
                            }
                        }
    ...
    var CreateBucketizedDateTimeFacets = function(facetName, array1, array2) {
    ...
    var CreateHiddenBucketizedDateTimeFacets = function(facetName, array1) {
    ...
    var CreateCustomRange = function(facetName) {
        var facetControls = ["<ul class='pv-filterpanel-accordion-facet-list'>"];
    ...
    var CreateDatetimeNoInfoFacet = function(facetName) {
        var facetControls = ["<ul class='pv-filterpanel-accordion-facet-list'>"];
    ...
    var CreateStringFacet = function(facetName) {
        var facetControls = ["<ul class='pv-filterpanel-accordion-facet-list'>"];
        for (var i = 0; i < _facetItemTotals.length; i++) {
    ...
    var CreateNumberFacet = function(facetName, facetValues) {
    ...
    var SelectView = function(viewNumber, init) {
        //Deselect all views
        for (var i = 0; i < _views.length; i++) {
            if (viewNumber != i) {
    ...
    var SortFacetItems = function(facetName) {
    ...
    var SelectStringFacetItem = function(facet, value) {
    ...
    var FilterCollection = function(changingView) {
        var filterItems = [];
        var foundItemsCount = [];
        var selectedFacets = [];
        ...
        for (var i = 0, _iLen = PivotCollection.FacetCategories.length; i < _iLen; i++) {
            if (PivotCollection.FacetCategories[i].Type == PivotViewer.Models.FacetType.Number) {
                ...
                if (sldr.length > 0) {
                    ...
                    var rangeMax = sldr.slider('option', 'max'),
                        rangeMin = sldr.slider('option', 'min');
                ...
                for (var k = 0, _kLen = datetimeFacets.length; k < _kLen; k++) {
                    ...
                    if (PivotCollection.Items[i].Facets[j].Name == datetimeFacets[k].facet) {
                        if (datetimeFacets[k].minDate && datetimeFacets[k].maxDate) {
                            ...
                            for (var n = 0, _nLen = datetimeFacets[k].facetValue.length; n < _nLen; n++) {
                                for (var l = 0; l < category.decadeBuckets.length; l++) {
                            ...
                            // year buckets
                            for (var n = 0, _nLen = datetimeFacets[k].facetValue.length; n < _nLen; n++) {
                                for (var l = 0; l < category.yearBuckets.length; l++) {
                            ...
                            // month buckets
                            for (var n = 0, _nLen = datetimeFacets[k].facetValue.length; n < _nLen; n++) {
                                for (var l = 0; l < category.monthBuckets.length; l++) {
                            ...
                            // day buckets
                            for (var n = 0, _nLen = datetimeFacets[k].facetValue.length; n < _nLen; n++) {
                                for (var l = 0; l < category.dayBuckets.length; l++) {
                            ...
                            // hour buckets
                            for (var n = 0, _nLen = datetimeFacets[k].facetValue.length; n < _nLen; n++) {
                                for (var l = 0; l < category.hourBuckets.length; l++) {
                            ...
                            // minute buckets
                            for (var n = 0, _nLen = datetimeFacets[k].facetValue.length; n < _nLen; n++) {
                                for (var l = 0; l < category.minuteBuckets.length; l++) {
                            ...
                            // second buckets
                            for (var n = 0, _nLen = datetimeFacets[k].facetValue.length; n < _nLen; n++) {
                                for (var l = 0; l < category.secondBuckets.length; l++) {
    ...
    var BucketCounts = function(bucketArray, name, itemsArray) {
    ...
    var GetDateTimeItemCounts = function(category, filterItems) {
    ...
    var FilterFacets = function(filterItems, selectedFacets) {
            ...
            for (var m = 0; m < PivotCollection.FacetCategories.length; m++) {
                if (PivotCollection.FacetCategories[m].IsFilterVisible) {
                                ...
                                if (facetCategory.IsFilterVisible) {
                                    for (var k = item.Facets[j].FacetValues.length - 1; k > -1; k -= 1) {
                                        ...
                                        if (facetCategory.Type == PivotViewer.Models.FacetType.String) {
                                            var filteredItem = {
                                                item: '#' + PivotViewer.Utils.EscapeMetaChars('pv-facet-item-' + CleanName(item.Facets[j].Name) + '__' + CleanName(item.Facets[j].FacetValues[k].Value)),
                                                count: 1
                                            };
                                            var found = false;
                                            ...
                                            if (!numFound) {
                                                numericFilterList.push({
                                                    Facet: item.Facets[j].Name,
                                                    Values: [item.Facets[j].FacetValues[k].Value]
                                                });
                                            }
                    ...
                    if (!hasValue) {
                        ...
                        var filteredItem = {
                            item: '#' + PivotViewer.Utils.EscapeMetaChars('pv-facet-item-' + CleanName(PivotCollection.FacetCategories[m].Name) + '__' + CleanName('(no info)')),
                            count: 1
                        };
                        var found = false;
                        for (var n = filterList.length - 1; n > -1; n -= 1) {
    ...
    var UpdateBreadcrumbNavigation = function(stringFacets, numericFacets, datetimeFacets) {
    ...
    var DeselectInfoPanel = function() {
        //de-select details
    ...
    var GetItemIds = function(facetName, value) {
        var foundId = [];
        for (var i = 0; i < PivotCollection.Items.length; i++) {
            var found = false;
    ...
    var UpdateBookmark = function() {
        ...
        if (_stringFacets.length > 0) {
            for (var i = 0; i < _stringFacets.length; i++) {
                for (var j = 0; j < _stringFacets[i].facetValue.length; j++) {
                    currentViewerState += "&";
        ...
        if (_numericFacets.length > 0) {
            for (var i = 0; i < _numericFacets.length; i++) {
                currentViewerState += "&";
        ...
        if (_datetimeFacets.length > 0) {
            for (var i = 0; i < _datetimeFacets.length; i++) {
                for (var j = 0; j < _datetimeFacets[i].facetValue.length; j++) {
                    currentViewerState += "&";
    ...
    var CleanName = function(uncleanName) {
        var cleanName = uncleanName.replace(/[^\w]/gi, '_');
        _nameMapping[cleanName] = uncleanName;
        return cleanName;
    }
        ...
        if (selectedItem != null) {
            ...
            for (var i = 0; i < selectedItem.Facets.length; i++) {
                ...
                if (IsMetaDataVisible) {
                    detailDOM[detailDOMIndex] = "<div class='pv-infopanel-detail " + (alternate ? "pv-detail-dark" : "pv-detail-light") + "'><div class='pv-infopanel-detail-item pv-detail-item-title' pv-detail-item-title='" + selectedItem.Facets[i].Name + "'>" + selectedItem.Facets[i].Name + "</div>";
                    for (var j = 0; j < selectedItem.Facets[i].FacetValues.length; j++) {
                        detailDOM[detailDOMIndex] += "<div pv-detail-item-value='" + selectedItem.Facets[i].FacetValues[j].Value + "' class='pv-infopanel-detail-item pv-detail-item-value" + (IsFilterVisible ? " pv-detail-item-value-filter" : "") + "'>";
                        if (selectedItem.Facets[i].FacetValues[j].Href != null) {
                            detailDOM[detailDOMIndex] += "<a class='pv-detail-item-link' href='" + selectedItem.Facets[i].FacetValues[j].Href + "'>" + selectedItem.Facets[i].FacetValues[j].Value + "</a>";
                        } else {
    ...
    $.subscribe("/PivotViewer/Views/ChangeTo/Grid", function(evt) {
        ...
        if (selectedTile) {
            $.publish("/PivotViewer/Views/Canvas/Click", [{
                x: selectedTile._locations[selectedTile.selectedLoc].destinationx + selectedTile.destinationwidth / 2,
                y: selectedTile._locations[selectedTile.selectedLoc].destinationy + selectedTile.destinationheight / 2
            }]);
        }
    });
    ...
    var AttachEventHandlers = function() {
        ...
        $('.pv-infopanel-details').on('click', '.detail-item-value-filter', function(e) {
            $.publish("/PivotViewer/Views/Item/Filtered", [{
                ...
                Item: this.getAttribute('pv-detail-item-value'),
                Values: null,
                ClearFacetFilters: true
            }]);
            return false;
        });
        ...
        $('.pv-infopanel-controls-navleft').on('click', function(e) {
            for (var i = 0; i < _filterItems.length; i++) {
                if (_filterItems[i].Id == _selectedItem.Id && _filterItems[i].Bucket == _selectedItemBkt) {
                    if (i >= 0) {
                        $.publish("/PivotViewer/Views/Item/Selected", [{
                            id: _filterItems[i - 1].Id,
                            bkt: _filterItems[i - 1].Bucket
                        }]);
                    }
                    ...
                    if (_currentView == 0 || _currentView == 1) {
                        for (var j = 0; j < _tiles.length; j++) {
                            if (_tiles[j].facetItem.Id == _filterItems[i - 1].Id) {
                                ...
                                var selectedCol = _views[_currentView].GetSelectedCol(_tiles[j], _filterItems[i - 1].Bucket);
                                var selectedRow = _views[_currentView].GetSelectedRow(_tiles[j], _filterItems[i - 1].Bucket);
        ...
        canvas.on('mousemove', function(evt) {
            ...
            if (_mouseDrag == null) {
                $.publish("/PivotViewer/Views/Canvas/Hover", [{
                    x: offsetX,
                    y: offsetY
                }]);
            } else {
            ...
            try {
                ...
                if (orig.touches.length > 1) {
                    ...
                    for (var i = 0; i < orig.touches.length; i++) {
                        helpers.push({
                            x: orig.touches[i].pageX,
                            y: orig.touches[i].pageY
                        });
    ...
    var FacetItemClick = function(checkbox) {
    ...
    var FacetSliderDrag = function(slider, min, max) {
    ...
    var Bucketize = function(bucketName, valueArray, itemId, bucketStartDate) {
        var found = false;
    ...
    var CreateDatetimeBuckets = function() {
        ...
        for (var i = 0; i < PivotCollection.FacetCategories.length; i++) {
            ...
            currentFacetCategory.decadeBuckets.sort(function(a, b) {
                return a.StartDate - b.StartDate
            });
            currentFacetCategory.yearBuckets.sort(function(a, b) {
                return a.StartDate - b.StartDate
            });
            currentFacetCategory.monthBuckets.sort(function(a, b) {
                return a.StartDate - b.StartDate
            });
            currentFacetCategory.dayBuckets.sort(function(a, b) {
                return a.StartDate - b.StartDate
            });
            currentFacetCategory.hourBuckets.sort(function(a, b) {
                return a.StartDate - b.StartDate
            });
            currentFacetCategory.minuteBuckets.sort(function(a, b) {
                return a.StartDate - b.StartDate
            });
            currentFacetCategory.secondBuckets.sort(function(a, b) {
                return a.StartDate - b.StartDate
            });
        }
    };

    var HideCustomDateRange = function(facetName) {
    ...
    var GetCustomDateRange = function(facetName) {
    ...
    var CustomRangeChanged = function(textbox) {
        var start;
        var end;
