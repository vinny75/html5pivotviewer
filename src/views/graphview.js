...
PivotViewer.Views.GraphView = PivotViewer.Views.TileBasedView.subClass({
    init: function() {
        ...
        $.subscribe("/PivotViewer/Views/Canvas/Zoom", function(evt) {
            ...
            if (that.rowscols.TileHeight < 10) {
                that.rowscols.TileHeight = 10;
            }
    ...
    Filter: function(dzTiles, currentFilter, sortFacet, stringFacets, changingView, changeViewSelectedItem) {
        ...
        $('.pv-viewarea-graphview-overlay div').fadeOut('slow', function() {
            $(this).remove();
        });
        ...
        for (var i = 0; i < bktArray.length; i++) {
            ...
            for (var j = 0; j < filterList.length; j++) {
                if (bktArray[i].Items.indexOf(filterList[j]) != -1) {
                    ...
                    if (newBkts.length == 0) {
                        newBkts.push({
                            startRange: bktArray[i].Name,
                            endRange: bktArray[i].Name,
                            Ids: [filterList[j]],
                            Values: [bktArray[i].Name]
                        });
                    } else {
    ...
    handleSelection: function(selectedItem, selectedTile, clickX, selectedLoc) {
        ...
        if (!found && !dontFilter) {
            ...
            $.publish("/PivotViewer/Views/Item/Filtered", [{
                Facet: that.sortFacet,
                Item: that.buckets[bucketNumber].startRange,
                MaxRange: that.buckets[bucketNumber].endRange,
                Values: that.buckets[bucketNumber].Values,
